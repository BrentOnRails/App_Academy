class TicTacToeNode
  ...
  atrr_reader :board, :next_player, :prev_move_pos

  def initialize(board, next_player, prev_move_pos = nil)
    @board, @next_player, @prev_move_pos =
    board, next_player, prev_move_pos
  end
...
class SuperComputerPlayer < ComputerPlayer
  def move(game, mark)
    ...
    node.children.each do |child_node|
      if best_child_node.nil?
        best_child_node = child_node
      elsif best_child_node.losing_node?(mark) &&
          !(child_node.losing_node?(mark))
        best_child_node = child_node
      elsif child_node.winning_node?(mark)
        best_child_node = child_node
      end
    end

    best_child_node.prev_move_pos
  end
end
